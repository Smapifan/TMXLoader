...
namespace TMXLoader
{
    public class TMXLoaderMod : Mod
    {
        ...
        {
            Harmony instance = new Harmony("Smapifan.TMXLoader.Entry");
        ...
        {
            Harmony instance = new Harmony("Smapifan.TMXLoader");

            void TryPatch(MethodBase original, MethodInfo prefix = null, MethodInfo postfix = null, string name = "unknown")
            {
                if (original == null)
                {
                    monitor.Log($"[Harmony] Original method for '{name}' not found.", LogLevel.Error);
                    return;
                }
                ...
                {
                    monitor.Log($"[Harmony] Failed to patch '{name}': {ex.Message}", LogLevel.Error);
                }
            }

            TryPatch(
                AccessTools.Method(typeof(SaveGame), nameof(SaveGame.loadDataToLocations)),
                postfix: AccessTools.Method(this.GetType(), nameof(SavePatch)),
                name: "SaveGame.loadDataToLocations"
            );

            TryPatch(
                AccessTools.Method(typeof(GameLocation), "setMapTile", new Type[] {
                    typeof(int), typeof(int), typeof(int), typeof(string), typeof(string), typeof(string), typeof(bool)
                }),
                prefix: AccessTools.Method(this.GetType(), nameof(trySetMapTile)),
                name: "GameLocation.setMapTile"
             );

            TryPatch(
                AccessTools.Method(typeof(Crop), nameof(Crop.harvest)),
                prefix: AccessTools.Method(this.GetType(), nameof(harvest)),
                postfix: AccessTools.Method(this.GetType(), nameof(harvestPost)),
                name: "Crop.harvest"
            );

            TryPatch(
                AccessTools.Method(typeof(HoeDirt), nameof(HoeDirt.performToolAction)),
                prefix: AccessTools.Method(this.GetType(), nameof(performToolAction)),
                name: "HoeDirt.performToolAction"
            );

            TryPatch(
                AccessTools.Method(typeof(HoeDirt), nameof(HoeDirt.draw), new[] { typeof(SpriteBatch) }),
                prefix: AccessTools.Method(this.GetType(), nameof(drawHoeDirt)),
                postfix: AccessTools.Method(this.GetType(), nameof(drawHoeDirtPost)),
                name: "HoeDirt.draw"
            );

            if (helper.ModRegistry.IsLoaded("Entoarox.FurnitureAnywhere"))
            {
                var FAType = Type.GetType("Entoarox.FurnitureAnywhere.ModEntry, FurnitureAnywhere");
                var sleepMethod = FAType?.GetMethod("SleepFurniture", BindingFlags.NonPublic | BindingFlags.Instance);
                var getFAMethod = this.GetType().GetMethod("GetFA", BindingFlags.Public | BindingFlags.Static);
                TryPatch(sleepMethod, prefix: getFAMethod, name: "FurnitureAnywhere.SleepFurniture");
            }
        }
        ...
        private static string TrySetMapTileLogKey = null;
        public static bool trySetMapTile(GameLocation __instance, int tileX, int tileY, int index, string layer, string tileSheetId, string action, bool copyProperties)
        {
            ...
            try
            {
                ...
                if (TrySetMapTileLogKey != logKey)
                {
                    TrySetMapTileLogKey = logKey;
                    monitor?.Log($"Setting map tiles for {__instance.NameOrUniqueName}...", LogLevel.Trace);
                }

                if (monitor.IsVerbose)
                {
                    monitor?.Log($"Setting MapTile: X:{tileX} Y:{tileY} Layer:{layer} TileSheet: {tileSheetId} Location:{__instance.Name}", LogLevel.Trace);
                }

                __instance.setMapTile(tileX, tileY, index, layer, tileSheetId, action, copyProperties);
                return false;
            }
            catch (Exception ex)
            {
                monitor?.Log($"Error setting MapTile at ({tileX},{tileY}) on layer '{layer}' with tilesheet '{tileSheetId}' in {__instance.Name}: {ex.Message}", LogLevel.Warn);
                monitor?.Log("----Tilesheets----", LogLevel.Trace);
                int i = 0;
                foreach (TileSheet ts in __instance.Map.TileSheets)
                    monitor?.Log($"{ts.Id} ({i++})", LogLevel.Trace);

                // Fallback: versuche nächsten Tilesheet zu verwenden
                try
                {
                    skipTrySetMapTile = true;
                    monitor?.Log("Trying fallback with next tilesheet...", LogLevel.Info);

                    int fallbackIndex = __instance.Map.TileSheets.ToList().FindIndex(t => t.Id == tileSheetId) + 1;
                    if (fallbackIndex < __instance.Map.TileSheets.Count)
                    {
                        string fallbackTileSheetId = __instance.Map.TileSheets[fallbackIndex].Id;
                        __instance.setMapTile(tileX, tileY, index, layer, fallbackTileSheetId, action, copyProperties);
                        monitor?.Log("...Fallback done", LogLevel.Info);
                    }
                    else
                    {
                        monitor?.Log("...No fallback tilesheet available", LogLevel.Error);
                    }
                }
                catch
                {
                    monitor?.Log("...Fallback failed", LogLevel.Error);
                }
        ...
        private void setTileActions()
        {
            ...
            PyUtils.addTileAction("DropIn", (key, values, location, position, layer) =>
            {
                ...
                if (st.Length > 1 && (Game1.GetPlayer(long.Parse(st[1]), onlyOnline: true) ?? Game1.MasterPlayer) != Game1.player)
                    return false;
        ...
        internal void loadPack(TMXContentPack tmxPack) 
        {
            ...
            PyLua.loadScriptFromFile(Path.Combine(Helper.DirectoryPath, "sr.lua"), "Smapifan.TMXLoader.SpouseRoom");
